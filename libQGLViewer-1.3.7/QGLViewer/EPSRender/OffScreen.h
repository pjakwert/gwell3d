...
//
#ifndef _OFFSCREEN_H_INCLUDED_
#define _OFFSCREEN_H_INCLUDED_
...
namespace OFFSCREEN {	// I put this here to avoid conflicts between Qt and
  ...
  public:
    ...
    OffScreenArea  (int type = UNKNOWN_OFFSCREEN_TYPE)
    {
      DefaultType = type ;
      i_Screen = -1;
      i_Height = 0;
  ...
  protected:
	...
	std::cerr << "GLError " << (void *)error << " encountered." << std::endl ;
      ...
      // Take into account the different versions of GL/glx.h
      ...
      static int pbAttribs[] = { GLX_LARGEST_PBUFFER, true,
				 ...
				 None };
#else
      static int pbAttribs[] = { GLX_LARGEST_BUFFER, true,
				 GLX_PRESERVED_CONTENTS, true,
				 GLX_MAX_PBUFFER_WIDTH,0,
				 GLX_MAX_PBUFFER_HEIGHT,0,
				 None };
#endif
      ...
      if (i_Drawable == 0)
	{
      ...
      if (i_pXVisual == NULL)
	// Driver problem, unable to allocate visual.
	return false;
