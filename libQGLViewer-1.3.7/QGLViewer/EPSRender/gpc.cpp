...
#define N_EDGE(d,e,p,i,j)  {(d)= (e); \
                            ...
                            (i)= (d)->bot.x + (d)->dx * ((j)-(d)->bot.y);}
/*
#define MALLOC(p, b, s)    {if( (sizeof (b)) > 0) { \
                            p= (b*)malloc(sizeof (b)); if (!(p)) { \
                            fprintf(stderr, "gpc malloc failure: %s\n", s); \
		            exit(0);}} else p= (b*)NULL;}
#define MALLOC_ARRAY(p, n, b, s)    {if( (n) * (sizeof (b)) > 0) { \
                            p= (b*)malloc((n) * (sizeof (b))); if (!(p)) { \
                            fprintf(stderr, "gpc array malloc failure: %s\n", s); \
		            exit(0);}} else p= (b*)NULL;}
#define FREE(p)            {if (p) {free(p); (p)= NULL;}}
*/
#define MALLOC(p, b, s)    {if( (sizeof (b)) > 0) { \
                            p=new b(); if (!(p)) { \
                            fprintf(stderr, "gpc new failure: %s\n", s); \
		            exit(0);}} else p= (b*)NULL;}
#define MALLOC_ARRAY(p, n, b, s)    {if( (n) * (sizeof (b)) > 0) { \
                            p=new b[(n)]; if (!(p)) { \
                            fprintf(stderr, "gpc new array failure: %s\n", s); \
		            exit(0);}} else p= (b*)NULL;}
...
static edge_node **bound_list(lmt_node **lmt, double y)
{
  ...
  if (!*lmt)
  {
    ...
    MALLOC(*lmt, lmt_node, "LMT insertion");
    (*lmt)->y= y;
...
static void add_to_sbtree(int *entries, sb_tree **sbtree, double y)
{
  if (!*sbtree)
  {
    ...
    MALLOC(*sbtree, sb_tree, "scanbeam tree insertion");
    (*sbtree)->y= y;
    (*sbtree)->less= NULL;
    (*sbtree)->more= NULL;
    (*entries)++;
                            ...
                            gpc_op op)
{
  ...
  MALLOC_ARRAY(edge_table, total_vertices, edge_node,
         "edge table creation");
                             ...
                             double x, double y)
{
  ...
  if (!*it)
  {
    ...
    MALLOC(*it, it_node, "IT insertion");
    (*it)->ie[0]= edge0;
    (*it)->ie[1]= edge1;
    (*it)->point.x= x;
    (*it)->point.y= y;
                        ...
                        double dy)
{
  ...
  if (!*st)
  {
    ...
    MALLOC(*st, st_node, "ST insertion");
    (*st)->edge= edge;
    (*st)->xb= edge->xb;
    (*st)->xt= edge->xt;
    (*st)->dx= edge->dx;
...
static void add_left(polygon_node *p, double x, double y)
{
  ...
  MALLOC(nv, vertex_node, "vertex node creation");
  nv->x= x;
  nv->y= y;
                          ...
                          double x, double y)
{
  ...
  MALLOC(*p, polygon_node, "polygon node creation");
...
static void add_vertex(vertex_node **t, double x, double y)
{
  if (!(*t))
  {
    MALLOC(*t, vertex_node, "tristrip vertex creation");
    (*t)->x= x;
    (*t)->y= y;
    (*t)->next= NULL;
  }
                         ...
                         double x, double y)
{
  if (!(*tn))
  {
    MALLOC(*tn, polygon_node, "tristrip node creation");
    (*tn)->next= NULL;
    (*tn)->v[LEFT]= NULL;
    (*tn)->v[RIGHT]= NULL;
    (*tn)->active= 1;
...
static bbox *create_contour_bboxes(gpc_polygon *p)
{
  ...
  MALLOC_ARRAY(box, p->num_contours, bbox, "Bounding box creation");
...
static void minimax_test(gpc_polygon *subj, gpc_polygon *clip, gpc_op op)
{
  ...
  MALLOC_ARRAY(o_table, subj->num_contours * clip->num_contours, int,
...
void gpc_read_polygon(FILE *fp, int read_hole_flags, gpc_polygon *p)
{
  ...
  MALLOC_ARRAY(p->hole, p->num_contours, int,
         "hole flag array creation");
  MALLOC_ARRAY(p->contour, p->num_contours, gpc_vertex_list, "contour creation");
  for (c= 0; c < p->num_contours; c++)
  {
    ...
    MALLOC_ARRAY(p->contour[c].vertex, p->contour[c].num_vertices, gpc_vertex, "vertex creation");
    for (v= 0; v < p->contour[c].num_vertices; v++)
...
void gpc_add_contour(gpc_polygon *p, gpc_vertex_list *new_contour, int hole)
{
  ...
  MALLOC_ARRAY(extended_hole, (p->num_contours + 1), int, "contour hole addition");

  /* Create an extended contour array */
  MALLOC_ARRAY(extended_contour, (p->num_contours + 1), gpc_vertex_list, "contour addition");
  ...
  MALLOC_ARRAY(extended_contour[c].vertex, new_contour->num_vertices, gpc_vertex, "contour addition");
                      ...
                      gpc_polygon *result)
{
  ...
  MALLOC_ARRAY(sbt, sbt_entries, double, "sbt creation");
  ...
  {
    MALLOC_ARRAY(result->hole, result->num_contours, int, "hole flag table creation");
    MALLOC_ARRAY(result->contour, result->num_contours, gpc_vertex_list, "contour creation");
    ...
    for (poly= out_poly; poly; poly= npoly)
    {
      ...
      if (poly->active)
      {
        ...
        MALLOC_ARRAY(result->contour[c].vertex,
                       ...
                       gpc_tristrip *result)
{
  ...
  {
    MALLOC_ARRAY(result->strip, result->num_strips, gpc_vertex_list,
    ...
    for (tn= tlist; tn; tn= tnn)
    {
      ...
      if (tn->active > 2)
      {
        ...
        MALLOC_ARRAY(result->strip[s].vertex, tn->active, gpc_vertex,
               "tristrip creation");
        v= 0;
        if (INVERT_TRISTRIPS)
        {
